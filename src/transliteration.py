...
# Mapping of the 33 modern Georgian characters (and capitals) to their Cyrillic approximations
...
def transliterate_text(text):
    ...
    result = ""
    for char in text:
        if char not in GEORGIAN_TO_CYRILLIC:
            logging.error(f"Cannot transliterate char '{char}': not Georgian.")
...
def transliterate_file(input_file, output_file):
    ...
    try:
        ...
        logging.info(f"Transliteration complete. Results saved to {output_file}")
        
    except Exception as e:
         logging.error(f"Error during transliteration: {e}")
